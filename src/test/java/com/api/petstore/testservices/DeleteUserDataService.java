...
public class DeleteUserDataService extends JsonFilesPath  {
	...
	public CreateUserResponse deleteUserData() throws IOException {
		...
		Response response = RestAssured.given()
				...
				.filter(new AllureRestAssured())
				.log().all()
				.delete(Endpoints.GET_DATA);


		String  getData =response.getBody().asString();

		createResponse = MAPPER.readValue(getData, CreateUserResponse.class);
